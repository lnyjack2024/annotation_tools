...
import { parseBoolean, parseNumber, parseOption } from "../parser";
import {
...
import { reviewFromColumnName } from "@/utils/consts";
...
export default class ConversationTemplate extends BaseTemplate<
  ConversationTemplateAttributes,
  unknown
> {
  static tabKeys = [
  ...
  type = TemplateType.LLM_CONVERSATION;
  ...
  private setAttributes(attr: any) {
    if (!attr) {
      this.attributes = {
        ...defaultAttributes,
      } as ConversationTemplateAttributes;
    } else {
      this.attributes = {};
      ...
      if (this.attributes.addible) {
        ...
        this.attributes.add_limit = parseNumber(attr.add_limit, -1, { min: 0 });
