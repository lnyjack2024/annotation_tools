...
declare class EventDispatcher<T> {
  ...
  on<K extends string>(
    name: K,
    callback: false | ((event: EditorEvent<MappedEvent<T, K>>) => void),
    prepend?: boolean,
    extra?: {},
  ): this;
...
declare type ArrayCallback$1<T, R> = (
  x: T,
  i: number,
  xs: ReadonlyArray<T>,
) => R;
declare type ObjCallback$1<T, R> = (
  value: T,
  key: string,
  obj: Record<string, T>,
) => R;
...
interface Tools {
  ...
  grep: {
    <T>(
      ...
      pred?: ArrayCallback<T, boolean>,
    ): T[];
    <T>(
      arr: Record<string, T> | null | undefined,
      pred?: ObjCallback<T, boolean>,
    ): T[];
  };
...
declare type DomQueryInitSelector<T extends Node> =
  | DomQuerySelector<T>
  | Window;
interface Hook {
...
interface DomQuery<T extends Node = Node> extends ArrayLike<T> {
  ...
  find<K extends keyof HTMLElementTagNameMap>(
    selector: K,
  ): DomQuery<HTMLElementTagNameMap[K]>;
  ...
  nextUntil(
    selector: DomQuerySelector<T>,
    until?: string,
  ): DomQuery<Node & ChildNode>;
...
interface BlobCache {
  ...
  findFirst: (
    predicate: (blobInfo: BlobInfo) => boolean,
  ) => BlobInfo | undefined;
...
declare type StyleFormat =
  | BlockStyleFormat
  | InlineStyleFormat
  | SelectorStyleFormat;
declare type AllowedFormat =
  | Separator
  | FormatReference
  | StyleFormat
  | NestedFormatting;
...
declare type CustomEditorInitFn = (
  elm: HTMLElement,
  settings: any,
...
declare type DialogFooterButtonSpec =
...
declare type DialogActionHandler<T> = (
  api: DialogInstanceApi<T>,
  details: DialogActionDetails,
) => void;
declare type DialogChangeHandler<T> = (
  api: DialogInstanceApi<T>,
...
declare type DialogTabChangeHandler<T> = (
  api: DialogInstanceApi<T>,
...
interface CardMenuItemSpec
  ...
  items: CardItemSpec[];
  onSetup?: (
    api: CardMenuItemInstanceApi,
  ) => (api: CardMenuItemInstanceApi) => void;
...
declare type AutocompleterContents =
  | SeparatorItemSpec
...
interface ToolbarButtonSpec
  extends BaseToolbarButtonSpec<ToolbarButtonInstanceApi> {
  type?: 'button';
...
interface BaseToolbarToggleButtonInstanceApi
...
interface ContextFormButtonSpec
  ...
  primary?: boolean;
  onAction: (
    formApi: ContextFormInstanceApi,
    api: ContextFormButtonInstanceApi,
  ) => void;
}
interface ContextFormToggleButtonSpec
  ...
  onAction: (
    formApi: ContextFormInstanceApi,
    buttonApi: ContextFormToggleButtonInstanceApi,
  ) => void;
  primary?: boolean;
}
...
declare type ContextMenuContents =
  | string
  | ContextMenuItem
...
declare type FancyMenuItemSpec =
...
type PublicInlineContent_d_ContextFormButtonInstanceApi =
  ContextFormButtonInstanceApi;
type PublicInlineContent_d_ContextFormToggleButtonSpec =
  ContextFormToggleButtonSpec;
type PublicInlineContent_d_ContextFormToggleButtonInstanceApi =
...
interface Registry$1 {
  ...
  getAll: () => {
    ...
    menuItems: Record<
      string,
      MenuItemSpec | NestedMenuItemSpec | ToggleMenuItemSpec
    >;
...
declare type FilePickerValidationStatus =
  | 'valid'
  | 'unknown'
  | 'invalid'
  | 'none';
...
declare type RunArguments<T extends Node = Node> =
  | string
  | T
  | Array<string | T>;
declare type BoundEvent = [Target, string, EventUtilsCallback<any>, any];
declare type Callback<K extends string> = EventUtilsCallback<
  MappedEvent<HTMLElementEventMap, K>
>;
interface DOMUtils {
  doc: Document;
  ...
  getParent: {
    ...
    <T extends HTMLElement>(
      node: string | Node,
      selector: (node: HTMLElement) => node is T,
      root?: Node,
    ): T | null;
  ...
  remove: <T extends Node>(
    node: string | T | T[] | DomQuery<T>,
...
declare type ParserFilterCallback = (
  nodes: AstNode[],
  name: string,
  args: ParserArgs,
) => void;
...
interface DomParser {
  ...
  addAttributeFilter: (
    name: string,
    callback: (nodes: AstNode[], name: string, args: ParserArgs) => void,
  ) => void;
...
declare type EditorCommandCallback = (
  ui: boolean,
  value: any,
  args: any,
) => void;
declare type EditorCommandsCallback = (
  command: string,
  ui: boolean,
  value: any,
  args: any,
...
declare class EditorCommands {
  ...
  addCommands<K extends keyof Commands>(
    commandList: Commands[K],
    type: K,
  ): void;
...
interface EditorManager extends Observable<EditorManagerEventMap> {
  ...
  overrideDefaults(
    this: EditorManager,
    defaultSettings: Partial<RawEditorSettings>,
  ): void;
...
declare class Editor implements EditorObservable {
  ...
  translate(text: Untranslated): TranslatedString;
  getParam<K extends keyof ParamTypeMap>(
    name: string,
    defaultVal: ParamTypeMap[K],
    type: K,
  ): ParamTypeMap[K];
  getParam<K extends keyof EditorSettings>(
    name: K,
...
declare type AddOnCallback<T> = (
  editor: Editor,
  url: string,
  $?: DomQueryConstructor,
) => void | T;
declare type AddOnConstructor<T> = new (
  editor: Editor,
  url: string,
  $?: DomQueryConstructor,
) => T;
...
declare type TextProcessCallback = (
  node: Text,
  offset: number,
  text: string,
) => number;
...
declare type WithSubItems<T, K extends keyof T> = T[K] extends Array<any>
  ? T & T[K][number]
  : T;
...
declare type ExtendedClass<T extends Props<A>, A extends any[]> = WithSubItems<
  T,
  'Mixins'
>;
interface ExtendedClassConstructor<T extends Props<A>, A extends any[] = any[]>
  extends Class {
...
interface Class {
  extend<T extends Props<A>, A extends any[] = any[]>(
    props: T,
  ): ExtendedClassConstructor<T, A>;
}
interface RGB {
  r: number;
  g: number;
